...
public class ColoredTriangleTest extends GLGame {
		...
		public class ColoredTriangleScreen extends Screen {
			final int VERTEX_SIZE = (2 + 4) * 4; // 2 co-ords 4 floats for colour and 4 bytes each
			...
			public ColoredTriangleScreen(Game game)
			{
				...
				//3 vertices of VERTEX SIZE
				ByteBuffer byteBuffer = ByteBuffer.allocateDirect(3 * VERTEX_SIZE);
				byteBuffer.order(ByteOrder.nativeOrder());
				vertices = byteBuffer.asFloatBuffer();
				vertices.put( new float[] { 
											0.0f, 0.0f, 1, 0, 0, 1,
											(glGraphics.getWidth() - 1), 0.0f, 0, 1, 0, 1,
											(glGraphics.getWidth() / 2),  (glGraphics.getHeight() - 1), 0, 0, 1, 1,
										  });
